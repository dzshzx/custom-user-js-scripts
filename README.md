...
| JavDB Recommend Archive / JavDB 佳片推荐 · 历史期数 | `https://javdb.com/*` 等 | 在 JavDB 导航栏加入「佳片推荐」入口，打开独立页面浏览全部历史期数（每周一/四更新），支持翻期、搜索与全期关键词搜索。 | [javdb-recommend.user.js](dist/javdb-recommend.user.js) |
...
- 瀑布流浏览：从上次浏览的期数开始按期流式渲染，期数选择框同步显示恢复的期数；可选择最新一期直接跳转。滚动到底自动加载更早的期数；相邻未加载期会沿当前流追加，远距离或反向跳转则直接以目标期重建流，不再请求中间期，并记住浏览位置。刷新期数与清缓存均保留浏览位置。
- 影片列表：横版封面完整显示（不裁切），卡片展示番号、评分与发售日期，点击直达官网影片详情页。
- 多脚本适配：若其他 JavDB 增强脚本接管影片列表，后续未接管列表会同步其实际列数、间距和封面模式；列表后来被接管时会释放本脚本复制的 inline 值。兼容范围按 Tampermonkey 与 `JAV老司机-新` 2.8.4.8 的公开 DOM 表现核对，不依赖第三方内部函数。
