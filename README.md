...
设置会显示当前站点的实际主机名；限制解除选项采用整行可点列表。控件提供键盘焦点提示，浮窗动效遵循系统的减少动态效果偏好。
...
悬浮入口采用实色背景和轻阴影；同步提示使用中性背景，以标题颜色区分成功或警告，支持明暗主题。
...
获取图片、读取图片、下载失败或超时时，会给出对应的中文操作提示；技术详情保留在浏览器控制台。
...
- 工具栏按浏览、搜索、维护分组，窄屏自动换行；清缓存为次要操作，控件保留清楚的键盘焦点提示。
