...
- 跨标签页时，同步只属于它开始时的目标（启用状态、token、Gist ID、文件名）。推送前重读 GM 设置，若另一标签页已关闭同步或更换了目标，本次不再推送并返回 `disabled` / `superseded`；结束时（成功或失败）只把 `gistId`、`lastSyncedAt`、`lastError` 写到存储中的最新设置上，目标已变化时丢弃本次结果，不恢复旧的启用状态或 token。已经发出的请求仍可能完成。
