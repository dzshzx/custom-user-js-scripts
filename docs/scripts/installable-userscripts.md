...
- 与其他 JavDB 增强脚本同时运行时，检测首个列表实际生效的列数和间距，并同步到后来动态追加的所有期数；脚本加载顺序不同也不会形成列数混排。
- 期数目录在页面 `localStorage` 缓存 6 小时；最新一期详情缓存 2 小时、历史详情缓存 30 天，详情缓存按最近使用保留最多 48 期。缓存损坏、过期或写入失败时明确退回官网接口。
