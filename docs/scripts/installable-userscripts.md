...
- 当前源码为 `javdb-recommend.entry.js` 与数据、请求、Period Section、展示 Lib Module，构建生成 Dist Bundle 和完整 Bridge File。数据模块拥有 Catalog、Navigation Payload、Search Index、Consumer Lease 及双路搜索；Period Section 统一浏览与搜索的安全文本、稳定节点、卡片、封面失败和即时筛选，展示模块编排顺序与导航。内存和磁盘均按来源时间判断 TTL，缓存投影不会延长来源有效期。
